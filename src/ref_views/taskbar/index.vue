<template>
  <section class="flex" id="taskbar">
    <i @click="toggleLeftMenu" class="icon-menu flex"
      ><svg-icon name="plasma"
    /></i>
    <div class="pinned flex">
      <app-icon
        v-for="app in pinnedApp"
        :class="'_item-' + app.name"
        :app="app"
      />
    </div>
    <div class="apps flex">
      <app-icon
        v-for="app in otherApp"
        :class="'_item-' + app.name"
        :app="app"
      />
    </div>
    ...
    <div class="pendant flex" :class="{ active: showRight === 'arrow' }">
      <i
        :class="{ active: showRight === 'paste' }"
        @click="toggleRightMenu('paste')"
      >
        <svg-icon name="paste" />
      </i>
      <i
        @click="
          keyboardType =
            keyboardType === 'keyboard' ? 'google-pinyin' : 'keyboard'
        "
      >
        <svg-icon :name="keyboardType" />
      </i>
      <i
        :class="{ active: showRight === 'volume' }"
        @click="toggleRightMenu('volume')"
      >
        <svg-icon name="audio-volume-muted" />
      </i>
      <i
        :class="{ active: showRight === 'bluetooth' }"
        @click="toggleRightMenu('bluetooth')"
      >
        <svg-icon name="bluetooth" />
      </i>
      <i
        :class="{ active: showRight === 'wifi' }"
        @click="toggleRightMenu('wifi')"
      >
        <svg-icon name="wifi" />
      </i>
      <i
        :class="{ toggled: showRight === 'arrow' }"
        @click="toggleRightMenu('arrow')"
        class="trans"
      >
        <svg-icon
          name="triangle"
          style="width: 1.1rem; height: 1.1rem; transform: rotate(-90deg)"
        />
      </i>
      <i @click="showDesktop">
...
export default defineComponent({
  ...
  computed: {
    pinnedApp(): Array<typeApp> {
      return this.apps
        .filter((app: typeApp) => app.taskbar != undefined)
        .sort((a: typeApp, b: typeApp) => a.taskbar - b.taskbar);
    },
    otherApp(): Array<typeApp> {
      return this.apps
        .filter(
          (app: typeApp) => app.order != undefined && app.status.value > 0,
        )
        .sort((a: typeApp, b: typeApp) => a.order.value - b.order.value);
    },
    time(): string {
    ...
    showDesktop() {
      this.apps.map((app: typeApp) => {
        if (this.showDesktopLis.includes(app.name)) {
          app.status.value = 2;
          this.showDesktopLis.splice(
            this.showDesktopLis.indexOf(app.name) || 0,
            1,
          );
...
@use "src/assets/style/public" as *;
