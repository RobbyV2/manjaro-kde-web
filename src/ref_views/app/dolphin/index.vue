  ...
  <div class="dolphin flex">
    <div class="head flex">
      <div class="view flex">
        ...
        <svg-icon
          v-for="i in ['icons', 'details', 'tree']"
          :name="'view-list-' + i"
        />
      </div>
      <span class="flex"
        >/<svg-icon name="arrow" /><span @click="dirNow = []">home</span
        ><svg-icon name="arrow" /><span @click="dirNow = []">user</span>
        <template v-for="(dir, idx) in dirNow">
          <svg-icon name="arrow" /><span
            @click="dirNow = dirNow.slice(0, idx + 1)"
            >{{ dir }}</span
          >
        </template>
      </span>
