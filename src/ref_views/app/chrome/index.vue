  ...
  <div class="chrome flex">
    <div class="tab flex">
      <div
        @click="activeIdx = idx"
        class="flex"
        :class="{ active: activeIdx === idx, before: activeIdx === idx + 1 }"
        v-for="(item, idx) in tabs"
      >
        <span>Tab {{ idx }}</span>
        <i @click.stop="closeTab(idx)" class="flex"
          ><svg-icon name="close"
        /></i>
      </div>
      <i
        @click="
          tabs.push('https://www.google.com/search?igu=1');
...
export default defineComponent({
  ...
  data() {
    return {
      tabs: ["https://www.google.com/search?igu=1", "https://google.com"],
      activeIdx: 0,
      url: "",
    };
  },
