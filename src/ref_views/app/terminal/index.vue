<template>
  <div class="terminal code-font" @click.self="focusInput">
    ...
    <div v-for="item in cmds">
      <b class="header" v-if="item[0] === 'cmd'"
        >[user@manjaro <span>{{ item[2] }}</span
        >]$</b
      >
      ...
      switch (this.cmdLis[0]) {
        ...
        case "help":
          ...
          this.cmds.push([
            "out",
            helps
              .map((v) => `<p>${escapeHtml(v[0])}</p>${escapeHtml(v[1])}\n`)
              .join(""),
          ]);
          break;
        case "ls":
          ...
          this.cmds.push([
            "out",
            this.filesNow
              .map((v) => (v.isDir ? `<span>${v.name}</span>` : v.name))
              .join("\n"),
          ]);
          break;
        case "poweroff":
          ...
          if (this.cmdLis.length > 2) {
            ...
            switch (dir) {
              ...
              case "~":
                ...
                break;
              case "..":
                if (this.dir.length > 0) this.dir.pop();
                break;
              case ".":
                break;
              default:
        ...
        case "I":
          this.pushCmd(this.input);
          if (
            this.cmdLis.length === 3 &&
            ["like", "love"].includes(this.cmdLis[1]) &&
            ["you", "u"].includes(this.cmdLis[2])
          ) {
            this.pushCmd(this.input);
            this.cmds.push([
              "out",
              `<a style="color: #ff7474">I ${this.cmdLis[1]} ${this.cmdLis[2]} too.😽</a>`,
            ]);
          } else {
        ...
        case "xdg-open":
          ...
          if (this.cmdLis.length > 2) {
            this.cmds.push([
              "out",
              'command "xdg-open" only take one argument',
            ]);
          } else {
            if (this.cmdLis[1]) {
              const file = this.filesNow.find(
                (v) => !v.isDir && v.name === this.cmdLis[1],
              );
              if (file) {
                const mime1 = file.mime.split("/")[0];
                switch (file.mime) {
                  case "text/markdown":
                    openApp.call(this, "gedit").params.filepath.value = this.dir
                      .concat([file.name])
                      .join("/");
                    break;
                  case "audio/mpeg":
                    openApp.call(this, "music").params.filepath.value = this.dir
                      .concat([file.name])
                      .join("/");
                    break;
                  default:
                    if (mime1 === "image") {
                      openApp.call(this, "image viewer").params.filepath.value =
                        this.dir.concat([file.name]).join("/");
                    }
                    break;
                }
              } else {
      ...
      if (!command) {
        ...
        if (needComplete) {
          for (const d of this.dirsNow) {
            if (
              d.name.length >= needComplete.length &&
              d.name.startsWith(needComplete)
            ) {
              dirs.push(d.name);
            }
          }
        ...
        if (command === "c") {
          ...
          ["r", "re", "reb", "rebo", "reboo", "reboot"].includes(command)
        ) {
          this.input = "reboot ";
        } else if (
          [
