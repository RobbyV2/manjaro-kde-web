  ...
  >
    <div>
      ...
      >
        ...
        <i class="maximize" @click="appMaximize"
          ><svg-icon :name="app.maxed.value ? 'rhomb' : 'arrow'"
        /></i>
        ...
        case 2:
          ...
          if (this.waitNormalize) {
            ...
            this.$el.style.transitionProperty =
              "top, left, opacity, transform, box-shadow";
            this.$el.style.top = `0`;
  ...
  methods: {
    ...
    resize(e: MouseEvent) {
      if (this.resizeX === -1 || this.resizeY === -1) {
        ...
        const delta = [
          this.resizeX ? e.screenX - this.prevPos[0] : 0,
          this.resizeY ? e.screenY - this.prevPos[1] : 0,
        ];
        this.doResize(delta);
      }
      this.prevPos = [e.screenX, e.screenY];
    ...
    doResize(e: Array<number>): boolean {
      const newSize = [this.app.size[0] + e[0], this.app.size[1] + e[1]];
